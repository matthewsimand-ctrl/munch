...
export default function ChefCompanion({ currentStep, totalSteps, timerRunning, isDone }: Props) {
  ...
  // Award XP when advancing to a new step
  useEffect(() => {
    if (!completedSteps.has(currentStep) && currentStep > 0) {
      const newCompleted = new Set(completedSteps);
      newCompleted.add(currentStep);
      setCompletedSteps(newCompleted);
      const xp = XP_PER_STEP;
      setSessionXp(prev => prev + xp);
      setXpPopup({ amount: xp, id: Date.now() });
    }
  }, [currentStep]);
  ...
  useEffect(() => {
    if (isDone && !completedSteps.has(-1)) {
      ...
      setXpPopup({ amount: bonus, id: Date.now() });
    ...
    <div className="space-y-2">
      ...
      <div className="relative w-full h-16 flex items-end">
        ...
        <motion.div
          ...
          animate={{
            left: `calc(1rem + ${Math.min(progress, 1)} * (100% - 3.5rem))`,
          }}
